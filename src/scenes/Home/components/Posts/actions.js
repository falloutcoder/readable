...
export const loadAllPosts = () => {
  return (dispatch) => {
    return getAllPosts().then(
        posts => dispatch({type: LOAD_ALL_POSTS, payload: posts})
    )
  }
}
...
export const loadAllPostsForCategory = (category) => {
  return (dispatch) => {
    return getAllPostsForCategory(category).then(
        posts => dispatch({type: LOAD_ALL_POSTS_FOR_CATEGORY, payload: posts})
    )
  }
}
